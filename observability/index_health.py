...
Corpus health report for policy-sop-assistant.
Checks BigQuery SOP chunk table freshness, ingestion failure rate,
...
CHUNKS_TABLE: str = os.environ.get(
    "CHUNKS_BQ_TABLE", f"{PROJECT_ID}.policy_sop.chunks"
)
DLQ_SUBSCRIPTION: str = os.environ.get(
    "DLQ_SUBSCRIPTION", f"projects/{PROJECT_ID}/subscriptions/policy-sop-dlq-sub"
)
...
def get_corpus_stats(client: bigquery.Client) -> dict:
    """Fetch chunk counts from BigQuery."""
    query = f"""
        SELECT
...
def get_parse_failure_rate(client: bigquery.Client) -> float:
    """Compute ingestion parse failure rate over the past 24 hours."""
    query = f"""
        SELECT
            COUNTIF(status = 'FAILED') / NULLIF(COUNT(*), 0) AS failure_rate
        FROM `{PROJECT_ID}.policy_sop.ingestion_jobs`
...
def build_health_report() -> CorpusHealthReport:
    ...
    logger.info(
        "Policy SOP corpus health: %s | lag=%.1f min | failures=%.1f%%",
        status,
        sync_lag or 0,
        failure_rate * 100,
    )
